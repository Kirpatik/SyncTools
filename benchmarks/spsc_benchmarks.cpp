...
struct BoostQueue
{
    boost::lockfree::spsc_queue<std::size_t, boost::lockfree::capacity<QUEUE_SIZE>> q;
...
BENCHMARK_TEMPLATE(BM_SPSCQueue, BoundedSPSCQueue<size_t, QUEUE_SIZE>)
    ->Args({0})
    ->Args({QUEUE_SIZE / 2})
    ->Args({QUEUE_SIZE - 1})
    ->UseRealTime()
...
BENCHMARK_TEMPLATE(BM_SPSCQueue, BoostQueue)
    ->Args({0})
    ->Args({QUEUE_SIZE / 2})
    ->Args({QUEUE_SIZE - 1})
    ->UseRealTime()

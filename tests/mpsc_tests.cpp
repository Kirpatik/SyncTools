...
TEST(MPSCQueueTests, Prefill1) { stress_test_queue<SyncTools::BoundedMPSCQueue<uint64_t, QUEUE_SIZE>>(1); }

TEST(MPSCQueueTests, PrefillHalf)
{
    stress_test_queue<SyncTools::BoundedMPSCQueue<uint64_t, QUEUE_SIZE>>(QUEUE_SIZE / 2);
}

TEST(MPSCQueueTests, PrefillAlmostFull)
{
    stress_test_queue<SyncTools::BoundedMPSCQueue<uint64_t, QUEUE_SIZE>>(QUEUE_SIZE - 1);
}

TEST(MPSCQueueTests, FIFOMixedAPIs)
{
    SyncTools::BoundedMPSCQueue<int, 8> q;
    for (int i = 0; i < 4; ++i)
    {
...
TEST(MPSCQueueTests, FullAndEmptyWithBlocking)
{
    SyncTools::BoundedMPSCQueue<int, 4> q;
    EXPECT_TRUE(q.empty());
    EXPECT_FALSE(q.full());
    ...
    EXPECT_TRUE(q.try_pop(y));
    EXPECT_EQ(y, 3);
    EXPECT_TRUE(q.blocking_pop(y));
    EXPECT_EQ(y, 4);
    EXPECT_TRUE(q.empty());
}

TEST(MPSCQueueTests, StopBlocksAll)
{
    SyncTools::BoundedMPSCQueue<int, 4> q;
    q.stop();
...
TEST(MPSCQueueTests, RunAfterStop)
{
    SyncTools::BoundedMPSCQueue<int, 4> q;
    q.stop();
    q.run();
...
TEST(MPSCQueueTests, StopDrainsRemaining)
{
    SyncTools::BoundedMPSCQueue<int, 4> q;
...
TEST(MPSCQueueTests, MoveOnlyObject)
{
    SyncTools::BoundedMPSCQueue<OnlyMovable, QUEUE_SIZE> queue;
...
TEST(MPSCQueueTests, CopyOnlyObject)
{
    SyncTools::BoundedMPSCQueue<OnlyCopyable, QUEUE_SIZE> queue;
...
TEST(MPSCQueueTests, NoDefaultConstructorObject)
{
    SyncTools::BoundedMPSCQueue<NoDefaultConstructor, QUEUE_SIZE> queue;
